...
$stmt = $pdo->prepare("SELECT * FROM apartments WHERE owner_id = ? AND status = 'occupied' AND title LIKE ?");
...
<title>Occupied Apartments</title>
<link rel="stylesheet" href="styles/style_owner_db.css">
</head>
<body>

<nav class="navbar">
    <div class="owner-title">Occupied Apartments</div>
    <div class="right-section">
        <form method="GET" action="occupied.php" style="display: flex;">
...
<main class="apartment-grid">
    <?php if (count($apartments) === 0): ?>
        <p style="grid-column: 1/-1; text-align:center; font-weight: bold;">No occupied apartments found.</p>
    <?php else: ?>
